...
public sealed class AuthService
{
    ...
    public async Task<string?> GetAdminVoiceSettingsAsync()
    {
        try
        {
            // Look for admin user settings
            var adminSettings = await _db.Users
                .Where(x => x.Roles.Contains("Admin"))
                .Select(x => x.VoiceSettings)
                .FirstOrDefaultAsync();
            
            Console.WriteLine($"[AuthService] Admin settings found: {adminSettings}");
            
            // If no admin user or no settings, create a default settings record
            if (string.IsNullOrWhiteSpace(adminSettings))
            {
                Console.WriteLine("[AuthService] No admin settings found, creating default settings record");
                
                // Check if admin user exists
                var adminUser = await _db.Users.FirstOrDefaultAsync(x => x.Roles.Contains("Admin"));
                
                if (adminUser != null)
                {
                    // Update existing admin user with default settings
                    var defaultSettingsJson = System.Text.Json.JsonSerializer.Serialize(new Dictionary<string, object>
                    {
                    ...
                    adminUser.VoiceSettings = defaultSettingsJson;
                    await _db.SaveChangesAsync();
                    Console.WriteLine("[AuthService] Created default settings for existing admin user");
                    return defaultSettingsJson;
                }
                else
                {
                    Console.WriteLine("[AuthService] No admin user found, returning null");
                    return null;
                }
            }
        ...
        {
            Console.WriteLine($"[AuthService] Error getting admin voice settings: {ex.Message}");
            return null;
        }
    }

    public async Task<bool> SaveAdminVoiceSettingsAsync(string settingsJson)
    {
        try
        {
            // Get the first admin user (there should typically be only one)
            var adminUser = await _db.Users
                .FirstOrDefaultAsync(x => x.Roles.Contains("Admin"));
            
            if (adminUser != null)
            {
                adminUser.VoiceSettings = settingsJson;
                await _db.SaveChangesAsync();
                return true;
            }
            
            // If no admin user found, create one for settings
            var (salt, hash) = PinHasher.HashPin("1234");
            var settingsUser = new AppUser
            {
                Name = "admin_voice_settings",
                Roles = "Admin",
                PinSalt = salt,
                PinHash = hash,
                VoiceSettings = settingsJson
            };
            
            _db.Users.Add(settingsUser);
            await _db.SaveChangesAsync();
            return true;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[AuthService] Error saving admin voice settings: {ex.Message}");
            return false;
        }
    }

    public async Task<string?> GetUserVoiceSettingsAsync(int userId)
    {
        try
        {
        ...
        {
            Console.WriteLine($"[AuthService] Error getting user voice settings: {ex.Message}");
            return null;
        }
    }

    public async Task<bool> SaveUserVoiceSettingsAsync(int userId, string settingsJson)
    {
        try
        {
            var user = await _db.Users.FindAsync(userId);
            if (user != null)
            {
                user.VoiceSettings = settingsJson;
                await _db.SaveChangesAsync();
                return true;
            }
            return false;
        ...
        {
            Console.WriteLine($"[AuthService] Error saving user voice settings: {ex.Message}");
            return false;
        }
    }
}
